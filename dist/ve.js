...
!function (t) {
	"use strict";
	function r(t) {
		var r = { path: !0, query: !0, hash: !0 };return t ? (/^[a-z]+:/.test(t) && (r.protocol = !0, r.host = !0, /[-a-z0-9]+(\.[-a-z0-9])*:\d+/i.test(t) && (r.port = !0), /\/\/(.*?)(?::(.*?))?@/.test(t) && (r.user = !0, r.pass = !0)), r) : r;
	}function e(t, e, o) {
		var u,
		    f,
		    l,
		    y = h ? "file://" + (process.platform.match(/^win/i) ? "/" : "") + p("fs").realpathSync(".") : document.location.href;e || (e = y), h ? u = p("url").parse(e) : (u = document.createElement("a"), u.href = e);var d = r(e);l = e.match(/\/\/(.*?)(?::(.*?))?@/) || [];for (f in a) {
			t[f] = d[f] ? u[a[f]] || "" : "";
			...
			var g = new n(y.match(/(.*\/)/)[0]),
			    m = g.path.split("/"),
			    v = t.path.split("/"),
			    q = ["protocol", "user", "pass", "host", "port"],
			    w = q.length;for (m.pop(), f = 0; w > f; f++) {
				t[q[f]] = g[q[f]];
			}for (; ".." === v[0];) {
				m.pop(), v.shift();
		...
		return t = t.replace(/\+/g, " "), t = t.replace(/%([ef][0-9a-f])%([89ab][0-9a-f])%([89ab][0-9a-f])/gi, function (t, r, e, o) {
			var s = parseInt(r, 16) - 224,
			    i = parseInt(e, 16) - 128;if (0 === s && 32 > i) return t;var n = parseInt(o, 16) - 128,
			    h = (s << 12) + (i << 6) + n;return h > 65535 ? t : String.fromCharCode(h);
		}), t = t.replace(/%([cd][0-9a-f])%([89ab][0-9a-f])/gi, function (t, r, e) {
			var o = parseInt(r, 16) - 192;if (2 > o) return t;var s = parseInt(e, 16) - 128;return String.fromCharCode((o << 6) + s);
		...
		for (var r, e = /([^=&]+)(=([^&]*))?/g; r = e.exec(t);) {
			var o = decodeURIComponent(r[1].replace(/\+/g, " ")),
			    i = r[3] ? s(r[3]) : "";void 0 !== this[o] && null !== this[o] ? (this[o] instanceof Array || (this[o] = [this[o]]), this[o].push(i)) : this[o] = i;
		}
	}function n(t, r) {
		e(this, t, !r);
	    ...
	    c = { ftp: 21, gopher: 70, http: 80, https: 443, ws: 80, wss: 443 };i.prototype.toString = function () {
		var t,
		    r,
		    e = "",
		    s = o;for (t in this) {
			if (!(this[t] instanceof Function || null === this[t])) if (this[t] instanceof Array) {
				var i = this[t].length;if (i) for (r = 0; i > r; r++) {
					e += e ? "&" : "", e += s(t) + "=" + s(this[t][r]);
				} else e += (e ? "&" : "") + s(t) + "=";
		...
		var r,
		    e = "",
		    i = 0;if (t && t.length && t + "" !== t) {
			for (this.isAbsolute() && (e = "/"), r = t.length; r > i; i++) {
				t[i] = !i && t[i].match(/^\w:$/) ? t[i] : o(t[i]);
			}this.path = e + t.join("/");
		}for (t = ("/" === this.path.charAt(0) ? this.path.slice(1) : this.path).split("/"), i = 0, r = t.length; r > i; i++) {
			t[i] = s(t[i]);
		}return t;
...
VeLib.helpers = {
	getParamsAsObjects: function getParamsAsObjects() {
		var url = new Url();
		return url.query;
	},

	outputMissingKeys: function outputMissingKeys(urlData) {
		Object.keys(urlData).map(function (key) {
			if (!urlData[key]) console.warn("Value for key ''" + key + "' is missing");
		});
	},
	fetchDefinition: function fetchDefinition() {
	...
	fetchTemplateData: function fetchTemplateData() {
...
VeLib.init = function () {
	...
	VeLib.ready = new Promise(function (resolve, reject) {
		Promise.all([VeLib.helpers.fetchDefinition().then(function (definition) {
			return VeLib.state.descriptor = definition;
		}), //.then(() => console.log(VeLib.state.descriptor))
		VeLib.helpers.fetchTemplateData().then(function (userData) {
			return VeLib.state.userData = userData;
